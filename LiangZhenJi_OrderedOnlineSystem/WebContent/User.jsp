...
<body>
        ...
        <nav>
            ...
            <ul id="nav_ul2" style="list-style: none;">
                ...
                <li >
                    ...
                    <a href="ShopCar.jsp?userkind=user">购物车</a>
                </li>
                <li >
                    <a>联系客服</a>
                </li>
        ...
        <section>
            ...
            <div><img id="logo1" src="img/logo1.jpg" alt="logo1"></div>
             <% if(request.getAttribute("information")!=null){ %>
				<script> alert("注册店铺信息已发送，请等待审核"); </script>
		 <%} %>
		  <%if(request.getAttribute("buyInformation")!=null){
            ...
            <div id="sosuokuang">
                ...
                <input type="submit" name="ensure" value="搜索" id="sosuo2" >
                 <input type="hidden" name="userKind" value="user">
            </form>
            <form  id="form2" action="">
                <p>筛选：</p>
                    <select id="sosuo0" name="kind1">
 ...
 <script type="text/javascript">
         ...
         %>
         <option value ="GoodsPageVeiw?pageNum=<%=i %>&sortKind=<%=request.getAttribute("sortKind")%>&userKind=user&msg=<%=request.getAttribute("searchInformation") %>"  >第<%=i %>页</option>
         <%} %>
         </select>
            </section>
            <%}else{ %>
         ...
         for(int i=1;i<=totalPage;i++){
         %>
         <option value ="ShopPageVeiw?pageNum=<%=i %>&sortKind=<%=request.getAttribute("sortKind")%>&userKind=user&msg=<%=request.getAttribute("searchInformation") %>"  >第<%=i %>页</option>
         <%} %>
         </select>
            </section>
            <%} %>
