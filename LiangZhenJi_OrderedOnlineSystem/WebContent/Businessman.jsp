...
<body>
    <div id="big_container">
        ...
        <nav>
            ...
            <ul id="nav_ul2" style="list-style: none;">
            	<li >
                    <a href="WeChat.jsp">买家消息</a>
                </li>
            	<li >
                    <select onchange="window.location=this.value">
                        <option value ="#">管理店铺</option>
                        <option value ="ScanDealOrderVeiw">处理订单</option>
                        <option value="AddGoods.jsp">上架商品</option>
                         <option value="ScanGoodsVeiw">管理商品</option>
                         <option value="GoodsManageVeiw">下架商品</option>
                    </select>
                </li>
                <li >
                    <select onchange="window.location=this.value">
                ...
                <li >
                    <img src="img/shopcar.png" alt="shopcar">
                     <a href="ShopCar.jsp?userKind=businessman">购物车</a>
                </li>
                <li >
                    <a >联系客服</a>
                </li>
        ...
        <section>
            ...
            <div><img id="logo1" src="img/logo1.jpg" alt="logo1"></div>
             <% if(request.getAttribute("addGoodsInformation")!=null){ %>
				<script> alert("商品信息已发送，请等待审核"); </script>
		 <%} %>
		 <%if(request.getAttribute("buyInformation")!=null){
            ...
            <div id="sosuokuang">
                ...
                <input type="hidden" name="userKind" value="businessman">
            </form>
 ...
 <script type="text/javascript">
         ...
         %>
         <option value ="GoodsPageVeiw?pageNum=<%=i %>&sortKind=<%=request.getAttribute("sortKind")%>&userKind=businessman&msg=<%=request.getAttribute("searchInformation") %>"  >第<%=i %>页</option>
         <%} %>
         </select>
            </section>
            <%}else{ %>
         ...
         for(int i=1;i<=totalPage;i++){
         %>
         <option value ="ShopPageVeiw?pageNum=<%=i %>&sortKind=<%=request.getAttribute("sortKind")%>&userKind=businessman&msg=<%=request.getAttribute("searchInformation") %>"  >第<%=i %>页</option>
         <%} %>
         </select>
            </section>
            <%} %>
